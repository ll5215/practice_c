...
int hasGreatGrandchild(BTNode *node)
{
    if (node == NULL)  // 노드가 NULL인 경우 (기저 조건)
        return 0;  // 깊이는 0
    ...
    // 현재 노드의 깊이를 계산 (왼쪽과 오른쪽 중 더 큰 깊이에 1을 더함)
    int currentDepth = (leftDepth > rightDepth ? leftDepth : rightDepth) + 1;

    // 현재 깊이가 3보다 큰 경우 (즉, 현재 노드가 증손자 노드를 가지고 있는 경우)
    if (currentDepth > 3) {
        printf("%d ", node->item);  // 현재 노드의 값을 출력
    }

    return currentDepth;  // 현재 노드의 깊이를 반환
}
