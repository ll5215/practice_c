...
int insertSortedLL(LinkedList *ll, int item) {   //7-31(수) 리팩토링
    int index = 0;  // 현재 인덱스
    ListNode *previousNode = NULL;  // 이전 노드 값 초기화
    ListNode *currentNode = ll->head;  // 현재 노드 값 초기화

    // 노드가 맨 처음에 삽입되는 경우
    if (currentNode == NULL || currentNode->item > item) {
        ListNode *newNode = malloc(sizeof(ListNode));  // 새로운 노드 생성
        newNode->item = item;  // 새로운 노드에 값 삽입
        newNode->next = currentNode;  // 새로운 노드는 다음 노드를 가리킴
        ll->head = newNode;  // 헤드 포인터가 새로운 노드를 가리킴
        ++(ll->size);  // 리스트 사이즈 증가
        return index;
    }
    ...
    while (currentNode != NULL && currentNode->item < item) {
        previousNode = currentNode;   // 이전 노드는 현재 노드를 가리킴
        currentNode = currentNode->next;  // 현재 노드는 다음 노드를 가리킴
        ++index;  // 인덱스 증가
    }
    ...
    if (previousNode != NULL) {
        previousNode->next = newNode;  // 이전 노드가 새로운 노드를 가리킴
    }
    newNode->next = currentNode;  // 새로운 노드는 현재 노드를 가리킴
    ++(ll->size);  // 리스트 사이즈 증가
