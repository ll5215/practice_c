...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
	...
	//삽입해야할 노드인 current,각각의 검색 직전 노드인 prevFront,prevBack
	ListNode *current,*previousFrontNode,*previousBackNode;
	
	if(ll == NULL)
		return;

	//current를 포인터헤더로,각각의 prevNode들은 NULL로 초기화
	current = ll->head;
	...
	while(current != NULL){
		...
		if(indexCount < sliceIndex){
			if(previousFrontNode == NULL){
				...
				previousFrontNode->next = current;
			}
			//직전 노드에 현재노드를 설정한다
			previousFrontNode = current;
			...
			if(previousBackNode == NULL){
				...
				previousBackNode->next = current;
			}
			previousBackNode = current;
			++(resultBackList->size);
		}
		//현재노드를 다음 현재노드로 이동시킨다
		current = current->next;
		++indexCount;
	}

	//현재노드를 의도하던 위치로 이동시킨 다음에는 기존에 있던 다음노드의 연결상태를 끊어야한다
